...
class MultiChainProvider {
    constructor() {
        ...
        const nativeApiUrl = isLocalDev ? 'http://localhost:3000' : 'https://cheeseblockchain.com';
        ...
        this.tokens = {
            ...
            'bsc': {
                ...
                'CHEESE': {
                    address: '0xf4ECd8c58Ec14e3AA4A0a2DDC33Bd3D5DEee73cd',
                    decimals: 18,
                    symbol: 'CHEESE',
                    name: 'CHEESE Token',
                    logo: 'https://raw.githubusercontent.com/cryptoexdevcheese/cheese-blockchain/master/public/wallet/icon-192.png'
                },
                'BUSD': {
    ...
    async getNativeBalance(address, networkId = null) {
        ...
        if (network === 'native') {
            ...
            try {
                const response = await fetch(`${config.rpc}/api/balance/${address}?apiKey=REDACTED_DEX_API_KEY`);
    ...
    async getAllBalances(address) {
        const balances = {
            native: {
                ...
                USDT: await this.getTokenBalance(address, 'USDT', 'native'),
                USDC: await this.getTokenBalance(address, 'USDC', 'native')
            },
            bsc: {
