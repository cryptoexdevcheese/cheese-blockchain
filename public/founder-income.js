...
class FounderIncome {
     ...
     * Note: Fees are ALWAYS paid in NCH (native currency), regardless of transaction currency
     */
    calculateTransactionFee(amount) {
        ...
        return {
            fee: fee,              // Fee in NCH
            netAmount: amount,     // FIXED: Full amount to recipient (fee is separate in NCH)
    ...
    async sendTransactionWithFee(from, to, amount, privateKey, data = {}) {
        try {
            ...
            // CRITICAL FIX: Check balances correctly
            // 1. User needs {amount} in the transaction currency (USDT/USDC/NCH)
            // 2. User needs {fee} in NCH (ALWAYS, regardless of transaction currency)
            const currency = data.currency || 'NCH';
            const currencyBalance = await this.api.getBalance(from, currency);

            // Check if user has enough of the transaction currency
            if (currencyBalance < amount) {
                throw new Error(`Insufficient ${currency} balance. Need ${amount} ${currency}, have ${currencyBalance}`);
            }

            // Check if user has enough NCH for the fee (skip if transaction is already NCH)
            if (currency !== 'NCH') {
                const nchBalance = await this.api.getBalance(from, 'NCH');
                if (nchBalance < feeCalc.fee) {
                    throw new Error(`Insufficient NCH for gas fee. Need ${feeCalc.fee} NCH, have ${nchBalance}`);
                }
            } else {
                // For NCH transactions, user needs amount + fee in NCH
                if (currencyBalance < amount + feeCalc.fee) {
                    throw new Error(`Insufficient NCH balance. Need ${amount + feeCalc.fee} NCH (amount + fee), have ${currencyBalance}`);
                }
            }
                ...
                try {
                    feeTx = await this.api.sendTransaction(
                        ...
                        privateKey,
                        {
                            ...
                            // CRITICAL FIX: Fees are ALWAYS paid in NCH, regardless of transaction currency
                            currency: 'NCH'
                        }
                    );
                } catch (feeError) {
     ...
     */
    async routeSwapFee(feeAmount, fromAddress, privateKey, swapTransactionId, currency = 'NCH') {
     ...
     */
    async routeBridgeFee(feeAmount, fromAddress, privateKey, bridgeTransactionId, currency = 'NCH') {
        if (!this.founderAddress || this.founderAddress === 'FOUNDER_WALLET_ADDRESS_HERE' || feeAmount <= 0) {
            return null;
        }
