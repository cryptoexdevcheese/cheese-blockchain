...
class EnhancedHybridBlockchainAI {
    constructor(options = {}) {
        ...
        this.miningReward = options.miningReward || 10;  // FIXED: 10 NCH (not 50)
        this.difficulty = options.difficulty || 4;       // FIXED: Difficulty 4 (not 2)
        this.smartContracts = [];
        ...
        // UPDATED 2026-01-09: Using FINAL core wallets
        this.founderAddress = options.founderAddress || '0x0E6ec6713E7b5b7C11d969dA848813d08223598E'; // FOUNDER
        this.founderPremine = options.founderPremine || 1000000; // 1M NCHEESE
        // CRITICAL: Treasury address - FINAL core treasury wallet
        this.treasuryAddress = options.treasuryAddress || '0x045D4e61757a873DAF5F3B59CCeD9f2585643cc3'; // TREASURY
        this.treasuryPremine = options.treasuryPremine || 2000000; // 2M NCHEESE
        // CRITICAL: Liquidity Pool address - FINAL core liquidity pool wallet
        this.liquidityPoolAddress = options.liquidityPoolAddress || '0x3801490C9f806c917b8CbA710Db9135FA3B116ae'; // LIQUIDITY
    ...
    async initialize() {
        ...
        // CRITICAL: Inject Native Stablecoins (USDT/USDC) Genesis
        await this.injectStablecoinGenesis();
    ...
    async minePendingTransactions(miningRewardAddress) {
        ...
        for (const tx of this.pendingTransactions) {
            if (tx.from) {
                ...
                if (balance < tx.amount) {
                    console.log(`🗑️ Removing invalid tx during mining: ${tx.from} has ${balance}, needs ${tx.amount}`);
        ...
        // Anomaly detection
        let anomalies = { hasAnomalies: false, anomalies: [], confidence: 0.95 };
        try {
            if (this.ml && typeof this.ml.detectAnomalies === 'function') {
                anomalies = await this.ml.detectAnomalies(block, this.chain);
            } else {
                console.warn('⚠️ ML detectAnomalies not available, using fallback');
            }
        } catch (error) {
            console.warn('⚠️ ML anomaly detection failed:', error.message);
        }
    ...
    // CRITICAL: Combined Balance & Portfolio for API
    async getBalances(address) {
        // Use getPortfolio logic to get everything
        const portfolio = this.getPortfolio(address);
        return {
            balance: portfolio['NCHEESE'] || 0,
            portfolio: portfolio
        };
    }

    // CRITICAL: Get full portfolio with all assets (Native NCHEESE + Native USDT/USDC)
        ...
        for (const block of this.chain) {
            if (block.transactions) {
                for (const tx of block.transactions) {
                    ...
                    // Determine asset type (Default: NCHEESE)
                    let asset = 'NCHEESE';
                    if (tx.data && tx.data.token) {
                        asset = tx.data.token.toUpperCase();
                    } else if (tx.data && tx.data.currency) {
                        asset = tx.data.currency.toUpperCase();
                    }

                    // CRITICAL FIX: Info Currency from Description if missing (Legacy Support)
                    // Some older transactions might lack the currency tag but have it in description
                    if (asset === 'NCHEESE' && tx.data && tx.data.description) {
                        const desc = tx.data.description.toUpperCase();
                        if (desc.includes('USDT')) {
                            asset = 'USDT';
                        } else if (desc.includes('USDC')) {
                            asset = 'USDC';
                        }
                    }

                    // HARD LOCK: USDT and USDC can NEVER be counted as NCHEESE
                    // This prevents phantom supply if metadata contains stablecoin keywords
                    if (asset === 'NCHEESE' && tx.data && tx.data.description) {
                        const desc = tx.data.description.toUpperCase();
                        if (desc.includes('USDT') || desc.includes('USDC')) {
                            continue; // Skip stablecoins when calculating NCHEESE
                        }
                    }

                    if (fromAddr === normalizedAddress) {
                        portfolio[asset] = (portfolio[asset] || 0) - (tx.amount || 0);
                    }
                    if (toAddr === normalizedAddress) {
                        portfolio[asset] = (portfolio[asset] || 0) + (tx.amount || 0);
                    }
    ...
    // CRITICAL: Inject Genesis Stablecoins if missing
    async injectStablecoinGenesis() {
        // Only inject if Treasury exists
        if (!this.treasuryAddress) return;

        // Check if already injected
        const hasUSDT = this.chain.some(b => b.transactions && b.transactions.some(tx =>
            tx.to === this.treasuryAddress && tx.data && tx.data.token === 'USDT' && tx.data.type === 'genesis_allocation'
        ));

        if (!hasUSDT) {
            console.log('💊 Injecting Genesis USDT/USDC for Treasury...');
            const genesisTime = this.chain[0] ? this.chain[0].timestamp : Date.now();
            ...
            const stablecoinTxs = [
                {
                    ...
                    data: { type: 'genesis_allocation', token: 'USDT', description: 'Genesis USDT Liquidity' },
                    signature: { publicKey: 'GENESIS', r: '0', s: '0' } // System tx
                },
                {
                    from: null,
                    to: this.treasuryAddress,
                    amount: 50000,
                    timestamp: genesisTime,
                    data: { type: 'genesis_allocation', token: 'USDC', description: 'Genesis USDC Liquidity' },
                    signature: { publicKey: 'GENESIS', r: '0', s: '0' } // System tx
                }
            ];

            // Add to a new block immediately
            const block = {
                index: this.chain.length,
                timestamp: Date.now(),
                transactions: stablecoinTxs,
                previousHash: this.getLatestBlock().hash,
                nonce: 0,
                difficulty: this.difficulty,
                aiValidation: { valid: true, agent: 'genesis_injector' }
            };

            block.hash = this.calculateHash(block);
            this.chain.push(block);
            await this.database.saveBlock(block);
            for (const tx of stablecoinTxs) {
                await this.database.saveTransaction(tx, block.index);
            }
            console.log('✅ Genesis USDT/USDC injected successfully (Block ' + block.index + ')');
        } else {
            console.log('✅ Genesis USDT/USDC already exists');
        }
